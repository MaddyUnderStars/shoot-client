...
export const SearchPane = ({ channel }: { channel: Channel }) => {
	const onSubmit: FormEventHandler<HTMLFormElement> = (e) => {
		...
		NiceModal.show(SearchResultsModal, { channel, query: query.toString() });
...
const SearchResultsModal = NiceModal.create(({ channel, query }: { channel: Channel, query: string }) => {
	...
	return <div className="h-full md:w-85 w-full z-10 absolute top-0 right-0 bg-sidebar">
		...
		{data?.messages.map(msg => <MessageComponent message={new Message(msg)} showControls={false} />)}
	</div>
})